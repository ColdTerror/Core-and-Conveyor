...
func _get_upgrade_stats(b: Building) -> Dictionary:
	...
	if "crafting_time_multiplier" in b and "crafting_time_multiplier" in temp and b.crafting_time_multiplier != temp.crafting_time_multiplier:
		stats["Craft Time Mult"] = "%.2fx -> %.2fx" % [b.crafting_time_multiplier, temp.crafting_time_multiplier]
	if "buffer_capacity" in b and "buffer_capacity" in temp and b.buffer_capacity != temp.buffer_capacity:
		stats["Buffer Capacity"] = "%d -> %d" % [b.buffer_capacity, temp.buffer_capacity]
	...
	if "attack_range" in b and "attack_range" in temp and b.attack_range != temp.attack_range:
		stats["Attack Range"] = "%.1f -> %.1f Tiles" % [b.attack_range / 32.0, temp.attack_range / 32.0]
	if "fire_rate" in b and "fire_rate" in temp and b.fire_rate != temp.fire_rate:
		stats["Fire Rate"] = "%.2f -> %.2f/s" % [b.fire_rate, temp.fire_rate]
	...
	if "projectiles_per_shot" in b and "projectiles_per_shot" in temp and b.projectiles_per_shot != temp.projectiles_per_shot:
		stats["Projectiles/Shot"] = "%d -> %d" % [b.projectiles_per_shot, temp.projectiles_per_shot]
	if "ammo_capacity" in b and "ammo_capacity" in temp and b.ammo_capacity != temp.ammo_capacity:
		stats["Ammo Capacity"] = "%d -> %d" % [b.ammo_capacity, temp.ammo_capacity]
